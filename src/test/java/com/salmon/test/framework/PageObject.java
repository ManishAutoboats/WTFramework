...
public abstract class PageObject {
    ...
    protected PageObject() {
        ...
        this.eyes = WebDriverHelper.getEyes();
    }
    ...
    // ******************************** // SCRAPERS // ************************************ //
    public void getAllBrokenLinkFromPage() throws Exception {
        List<WebElement> linkList;
        ArrayList<String> activeLinks;
        HttpURLConnection connection;

        linkList = webDriver.findElements(By.tagName("a"));

        activeLinks  = new ArrayList<>();
        for (WebElement link : linkList) {
            boolean validLink = link.getAttribute("href") != null && !link.getAttribute("href").contains("javascript") && !link.getAttribute("href").contains("tel") && !link.getAttribute("href").contains("mailto") && link.getAttribute("href").contains("vype");
            if (validLink) {
    ...
    // ******************************** // ASSERTIONS // ************************************ //
    public void assertTrueExpectedTextContainsActualTextWithCustomError(String expectedText, String actualText){
        assertTrue("\n\n ******** ERROR *********** \n" +
                "\n NOT THE EXPECTED RESULT!! " +
                "\n EXPECTED : " + expectedText +
                "\n ACTUAL   : " + actualText,expectedText.contains(actualText));
    }
    ...
    public void waitForPage() {
        new WebDriverWait(webDriver, 10).until(webDriver ->
...
/*    public void waitForExpectedElementIgnoreStale(final By by) {
    ...
    public void eyesCheckVisiblePage(String tag) {
        if (Props.EYES_ON) {
            boolean forceFullPage = eyes.getForceFullPageScreenshot();
            eyes.setForceFullPageScreenshot(false);
            eyes.check(tag, Target.window().fully(false));
            eyes.setForceFullPageScreenshot(forceFullPage);
        }
    }

    public void eyesCheckFullyPage(String tag) {
        if (Props.EYES_ON) {
    ...
    public void eyesCheckFullyPageIgnoreDisplacements(String tag) {
        if (Props.EYES_ON) {
            scrollToShowEntirePage();
            eyes.check(tag, Target.window().fully().ignoreDisplacements());
        }
    }

    public void eyesCheckRegion(String tag, By byRegion, boolean fully) {
        if (Props.EYES_ON) {
            waitForExpectedElement(byRegion, 10);
            if (fully) {
                eyes.check(tag, Target.region(byRegion).fully());
            } else {
                boolean forceFullPage = eyes.getForceFullPageScreenshot();
                eyes.setForceFullPageScreenshot(false);
                eyes.check(tag, Target.region(byRegion));
                eyes.setForceFullPageScreenshot(forceFullPage);
            }
        }
    }

    public void eyesCheckRegion(String tag, By byRegion) {
        eyesCheckRegion(tag, byRegion, true);
    }

    public void eyesCheckScrollRegion(String tag, By byRegion) {
        if (Props.EYES_ON) {
            waitForExpectedElement(byRegion, 10);
            eyes.check(tag, Target.region(byRegion).scrollRootElement(byRegion));
        }
    }
    ...
    public void scrollToShowEntirePage() {
        // ignore the statement for vuseit to pass its Eyes test
    ...
    protected LocalDate getDateFromString(String date) throws ParseException {
        ...
        switch(UrlBuilder.getLocale()){
            ...
            case "lyftse":
            case "it":
                monthStr=convertMonthString(date.replace(",","").split(" ")[0]);
                dayStr=date.replace(",","").split(" ")[1];
                yearStr=date.replace(",","").split(" ")[2];
                break;
            default:
                monthStr=convertMonthString(date.split(" ")[1]);
                dayStr=date.split(" ")[0];
    ...
    private String convertMonthString(String monthStr){
